...
class TestPasswordChangeSerializer(TestCase):
    ...
    def setUpTestData(cls):
        ...
        request_data = {"new_password1": "Password", "new_password2": "Password"}
    ...
    def test_custom_validation(self):
        ...
        serializer = PasswordChangeSerializer(
            reverse("rest_password_change"), data=self.request_data
        )
        serializer.validate(self.request_data)
        PasswordChangeSerializer.custom_validation.assert_called_once_with(
            self.request_data
        )
...
class TestSocialLoginSerializer(TestCase):
    NO_VIEW_SUBMIT_ERROR = {
        "non_field_errors": ["View is not defined, pass it as a context variable"]
    }
    ...
    def test_validate_no_view_submit(self):
        serializer = SocialLoginSerializer(
            data=self.request_data, context={"request": self.request}
        )
        serializer.is_valid()
    ...
    def test_validate_no_adapter_class_present(self):
        ...
        serializer = SocialLoginSerializer(
            ...
            context={"request": self.request, "view": dummy_view},
        )
        serializer.is_valid()
