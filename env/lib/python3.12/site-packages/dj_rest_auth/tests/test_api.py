...
class APIBasicTests(TestsMixin, TestCase):
    ...
    def test_login_failed_email_validation(self):
        ...
        self.assertEqual(
            resp.json["non_field_errors"][0], 'Must include "email" and "password".'
        )
    ...
    def test_login_failed_username_validation(self):
        ...
        resp = self.post(self.login_url, data=payload, status_code=400)
        self.assertEqual(
            resp.json["non_field_errors"][0], 'Must include "username" and "password".'
        )
    ...
    def test_allauth_login_with_email(self):
        ...
        get_user_model().objects.create_user(
            self.EMAIL, email=self.EMAIL, password=self.PASS
        )
    ...
    @override_api_settings(
        REGISTER_PERMISSION_CLASSES=("tests.mixins.CustomPermissionClass",)
    )
    def test_registration_with_custom_permission_class(self):
        ...
        request = factory.post(
            "/customer/details", self.REGISTRATION_DATA, format="json"
        )

        response = CustomRegisterView.as_view()(request)
        self.assertEqual(
            response.data["detail"], api_settings.REGISTER_PERMISSION_CLASSES[0].message
        )
    ...
    def test_wo_csrf_enforcement(self):
        ...
        self.assertEqual(resp.status_code, 200)
        resp = jwtclient.post(
            "/protected-view/", {}, HTTP_AUTHORIZATION="Bearer " + token
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_login_with_no_token_model(self):
        ...
        # there is no users in db so it should throw error (400)
        resp = self.post(
            self.login_url, data=payload, status_code=status.HTTP_400_BAD_REQUEST
        )

        get_user_model().objects.create_user(self.USERNAME, "", self.PASS)
        resp = self.post(
            self.login_url, data=payload, status_code=status.HTTP_204_NO_CONTENT
        )
    ...
    def test_misconfigured_token_model(self):
        ...
        with override_api_settings(
            SESSION_LOGIN=False, USE_JWT=False, TOKEN_MODEL=False
        ):
            with self.assertRaises(ImproperlyConfigured):
                get_token_model()
        ...
        with override_api_settings(
            SESSION_LOGIN=True, USE_JWT=False, TOKEN_MODEL=False
        ):
            assert get_token_model() is False

        # only jwt authentication is fine
        with override_api_settings(
            SESSION_LOGIN=False, USE_JWT=True, TOKEN_MODEL=False
        ):
            assert get_token_model() is False
